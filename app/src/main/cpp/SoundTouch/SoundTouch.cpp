...
uint SoundTouch::numProcessedSamples() const
{
    FIFOSamplePipe *psp = NULL;

    if (output == pTDStretch)
    {
        psp = pTDStretch->getOutput();
        if (psp)
        {
//            LOGI("SoundTouch::numProcessedSamples() pTDStretch numSamples = %d", psp->numSamples());
            return psp->numSamples();
        }
    }
    else if (output == pRateTransposer)
    {
        psp = pRateTransposer->getOutput();
        if (psp)
        {
//            LOGI("SoundTouch::numProcessedSamples() pRateTransposer numSamples = %d", psp->numSamples());
            return psp->numSamples();
        }
    }
