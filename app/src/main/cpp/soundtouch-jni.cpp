...
static void* play_direct_buf_address = NULL;
static void* rec_direct_buf_address = NULL;
...
extern "C" DLL_PUBLIC void Java_net_surina_soundtouch_SoundTouch_cachePlayDirectBufferAddress(JNIEnv* env, jobject thiz, jobject byte_buffer)
{
    play_direct_buf_address = env->GetDirectBufferAddress(byte_buffer);
}

extern "C" DLL_PUBLIC void Java_net_surina_soundtouch_SoundTouch_cacheRecordDirectBufferAddress(JNIEnv* env, jobject thiz, jobject byte_buffer)
{
    rec_direct_buf_address = env->GetDirectBufferAddress(byte_buffer);
}

extern "C" DLL_PUBLIC void Java_net_surina_soundtouch_SoundTouch_putSamples(JNIEnv* env, jobject thiz, jlong handle, jint numSamples)
{
    SoundTouch *ptr = (SoundTouch*)handle;
    if (ptr)
    {
//        LOGV("putSamples nSamples = %d ", numSamples);
        ptr->putSamples((SAMPLETYPE*)rec_direct_buf_address, numSamples);
    }
}

extern "C" DLL_PUBLIC int Java_net_surina_soundtouch_SoundTouch_numSamples(JNIEnv* env, jobject thiz, jlong handle)
{
    SoundTouch *ptr = (SoundTouch*)handle;
    if (ptr)
    {
        return ptr->numProcessedSamples();
    }

    return 0;
}

extern "C" DLL_PUBLIC int Java_net_surina_soundtouch_SoundTouch_receiveSamples(JNIEnv* env, jobject thiz, jlong handle, jint maxSamples)
{
    SoundTouch *ptr = (SoundTouch*)handle;
    if (ptr)
    {
        int nSamples = 0;
//        LOGV("numProcessedSamples = %d", ptr->numProcessedSamples());
        if (ptr->numProcessedSamples() >= maxSamples)
        {
            nSamples = ptr->receiveSamples((SAMPLETYPE*)play_direct_buf_address, maxSamples);
        }

//        LOGV("receiveSamples nSamples = %d", nSamples);
        return nSamples;
    }
...
extern "C" DLL_PUBLIC void Java_net_surina_soundtouch_SoundTouch_setTempoChange(JNIEnv *env, jobject thiz, jlong handle, jfloat tempo)
{
    SoundTouch *ptr = (SoundTouch*)handle;
...
extern "C" DLL_PUBLIC void Java_net_surina_soundtouch_SoundTouch_setRate(JNIEnv *env, jobject thiz, jlong handle, jfloat speed)
{
	SoundTouch *ptr = (SoundTouch*)handle;
	ptr->setRate(speed);
}

extern "C" DLL_PUBLIC void Java_net_surina_soundtouch_SoundTouch_setRateChange(JNIEnv *env, jobject thiz, jlong handle, jfloat speed)
{
    SoundTouch *ptr = (SoundTouch*)handle;
    ptr->setRateChange(speed);
}

extern "C" DLL_PUBLIC jboolean Java_net_surina_soundtouch_SoundTouch_setSetting(JNIEnv *env, jobject thiz, jlong handle, jint settingId, jint value)
{
    SoundTouch *ptr = (SoundTouch*)handle;
    return ptr->setSetting(settingId, value);
}

extern "C" DLL_PUBLIC jint Java_net_surina_soundtouch_SoundTouch_getSetting(JNIEnv *env, jobject thiz, jlong handle, jint settingId)
{
    SoundTouch *ptr = (SoundTouch*)handle;
    return ptr->getSetting(settingId);
}
